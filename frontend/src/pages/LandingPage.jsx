...
function IconKependudukan() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="1.8" className="w-5 h-5 transition-transform duration-300 group-hover:scale-110">
...
function IconNetwork() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="#2563eb" strokeWidth="1.8" className="w-7 h-7 mx-auto transition-transform duration-500 group-hover:rotate-12">
...
function IconDataset() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="1.8" className="w-7 h-7 mx-auto transition-transform duration-500 group-hover:scale-110">
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen text-slate-800 relative overflow-hidden"
      style={{ background: '#ffffff', fontFamily: "'Segoe UI', 'Inter', sans-serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 animate-slide-down ${scrolled ? 'backdrop-blur-md shadow-lg border-b border-indigo-500/20' : 'border-b border-indigo-500/10'}`} style={{ background: 'linear-gradient(to right, #2563eb, #4f46e5)', transition: 'all 0.3s' }}>
        <div className="max-w-4xl mx-auto px-6 py-3.5 flex items-center justify-between">
          {/* Logo */}
          <div onClick={() => navigate('/')} className="cursor-pointer flex items-center hover:opacity-85 transition-opacity bg-white rounded-lg px-2.5 py-1 shadow-sm">
          ...
          <div className="hidden md:flex items-center gap-8 text-xs sm:text-sm font-semibold">
            ...
            <button onClick={() => navigate('/panduan')} className="text-blue-100 hover:text-white transition-colors bg-transparent border-none cursor-pointer font-semibold text-xs sm:text-sm">Panduan</button>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-white hover:bg-slate-100 text-blue-900 font-extrabold px-5 py-2.5 rounded-lg text-xs sm:text-sm flex items-center gap-2 transition-all shadow-md shadow-blue-950/20 cursor-pointer active:scale-95"
            >
              {loginLabel} <ArrowRight size={14} />
            </button>
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className="md:hidden p-2 text-blue-100 hover:text-white cursor-pointer rounded-lg hover:bg-blue-900/40 transition-colors"
            >
        ...
        {menuOpen && (
          <div className="md:hidden bg-blue-900 border-t border-blue-800 px-6 py-4 space-y-3 shadow-lg text-sm font-semibold text-white">
            <a href="#beranda" onClick={() => setMenuOpen(false)} className="block text-white py-1">Beranda</a>
            <button onClick={() => { navigate('/panduan'); setMenuOpen(false); }} className="block text-left w-full text-blue-200 hover:text-white py-1 bg-transparent border-none font-semibold">Panduan</button>
          </div>
        )}
      </nav>

      {/* ══════════════════ HERO SECTION WRAPPER ══════════════════ */}
      <div className="w-full relative border-b border-slate-100/80" style={{ background: 'linear-gradient(135deg, #e0f2fe 0%, #eef2ff 100%)' }}>
        {/* Premium Decorative Gradient Blobs */}
        <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full bg-blue-600/5 blur-[130px] pointer-events-none -mr-40 -mt-20"></div>
        <div className="absolute top-[300px] left-0 w-[500px] h-[500px] rounded-full bg-indigo-500/5 blur-[120px] pointer-events-none -ml-40"></div>

        {/* Offset */}
        <div className="h-16"></div>

        {/* ══════════════════ HERO ══════════════════ */}
        <section id="beranda" className="py-16 sm:py-20 px-6 max-w-4xl mx-auto animate-fade-in-up">
        <div className="flex flex-col md:flex-row items-center gap-12">

          {/* Left Text Column */}
          <div className="flex-1 space-y-6 text-center md:text-left">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200/60 shadow-sm animate-pulse">
              <svg viewBox="0 0 24 24" fill="none" stroke="#137333" strokeWidth="3" className="w-3.5 h-3.5 shrink-0">
                <path d="M20 6L9 17l-5-5" />
              </svg>
              <span className="text-[10px] font-bold tracking-wider text-emerald-800 uppercase">Sistem Aktif & Terlindungi</span>
            </div>

            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-950 leading-tight min-h-[90px] sm:min-h-[120px]">
            ...
            <p className="text-xs sm:text-sm text-slate-500 leading-relaxed max-w-md mx-auto md:mx-0">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-2">
              <button
                ...
                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-extrabold px-6 py-3 rounded-xl text-xs sm:text-sm flex items-center gap-2.5 transition-all shadow-lg shadow-blue-600/20 hover:shadow-blue-600/35 cursor-pointer active:scale-95"
              >
                Mulai Sekarang
              ...
              <button
                ...
                className="bg-white hover:bg-slate-50 text-slate-700 font-bold px-6 py-3 rounded-xl text-xs sm:text-sm flex items-center gap-2 border border-slate-300 hover:border-slate-400 transition-all shadow-sm cursor-pointer active:scale-95"
              >
          ...
          {/* Right Column: Live Data Gathering Animation (Moved Here!) */}
          <div className="flex-1 flex justify-center w-full max-w-sm md:max-w-none">
            <div className="relative group p-2 w-full max-w-[340px]">
              ...
              <div className="absolute inset-0 bg-blue-600/10 rounded-3xl blur-xl group-hover:bg-blue-600/15 transition-all duration-500"></div>

              {/* Animated Gathering Container Box */}
              <div className="animated-hero-container relative w-full bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl overflow-hidden flex flex-col justify-center items-center" style={{ minHeight: '260px' }}>
      ...
      <section className="py-8 px-6 max-w-4xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div
            id="stat-card-1"
            className={`group bg-white hover:bg-blue-50/20 border border-slate-200/80 rounded-2xl p-8 text-center shadow-sm hover-card-effect hover-blue-effect cursor-pointer transition-all duration-1000 transform ${statsVisible1 ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'
              }`}
          >
            <div className="inline-flex p-3 bg-blue-50 rounded-xl mb-4 group-hover:scale-110 transition-transform">
              <IconNetwork />
            </div>
            <div className="text-3xl font-black text-blue-950 mb-1">{dbStats.opds}</div>
            <div className="text-xs text-slate-500 font-bold tracking-wider uppercase">OPD Terintegrasi</div>
          </div>

          {/* Card 2: Dataset Sektoral */}
          <div
            id="stat-card-2"
            className={`group bg-white hover:bg-emerald-50/20 border border-slate-200/80 rounded-2xl p-8 text-center shadow-sm hover-card-effect hover-green-effect cursor-pointer transition-all duration-1000 transform ${statsVisible2 ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'
              }`}
          >
            <div className="inline-flex p-3 bg-emerald-50 rounded-xl mb-4 group-hover:scale-110 transition-transform">
              <IconDataset />
            </div>
            <div className="text-3xl font-black text-emerald-700 mb-1">{dbStats.endpoints}</div>
            <div className="text-xs text-slate-500 font-bold tracking-wider uppercase">Dataset Sektoral</div>
          </div>
        </div>
      </section>

      {/* ══════════════════ TOPIK DATA ══════════════════ */}
      <section id="panduan" className="py-12 px-6 max-w-4xl mx-auto animate-fade-in-up delay-200">
        <div>
          {/* Section header */}
          <div className="flex items-center gap-3.5 mb-8">
            <div className="p-2.5 bg-blue-500/10 rounded-xl border border-blue-500/20 flex items-center justify-center">
            ...
            <div>
              <h2 className="text-lg font-extrabold text-slate-900 leading-none">Topik Layanan Data</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {topics.map(({ Icon, label }, i) => (
              <button
                key={i}
                onClick={() => navigate(loginDest)}
                className="group bg-white hover:bg-blue-50/50 border border-slate-200/80 rounded-2xl p-5 flex items-center gap-4 cursor-pointer text-left text-xs sm:text-sm font-semibold text-slate-800 hover:text-blue-900 shadow-sm hover-card-effect"
              >
                <div className="w-10 h-10 rounded-xl bg-blue-50/60 border border-blue-100 flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:bg-blue-100/80">
                  <Icon />
                </div>
      ...
      {/* ══════════════════ PREMIUM ROYAL BLUE FOOTER ══════════════════ */}
      <footer className="py-16 px-6 text-slate-200 relative overflow-hidden" style={{ background: 'linear-gradient(to right, #2563eb, #4f46e5)' }}>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 relative z-10">

          {/* Column 1 - Brand description */}
          <div className="space-y-4">
            <div className="flex items-center">
              <img src="/logo_apiget.png" alt="APIGATE Logo" className="h-10 w-auto object-contain bg-white rounded-lg p-1" />
            </div>
            ...
            <p className="text-[10px] text-blue-200">© 2024 Pemerintah Kabupaten Lampung Utara. All Rights Reserved.</p>
          </div>

          {/* Column 2 - Quick Links */}
          <div className="space-y-4">
            <div className="text-xs font-bold text-white uppercase tracking-wider">Tautan Penting</div>
            <ul className="space-y-2.5 text-xs">
              {['Kebijakan Privasi', 'Syarat & Ketentuan', 'Keamanan Informasi', 'Peta Situs'].map(link => (
                <li key={link}>
                  <a href="#" className="text-blue-100 hover:text-white transition-colors decoration-transparent">
                    {link}
                  </a>
                </li>
              ))}
          ...
          <div className="space-y-4">
            <div className="text-xs font-bold text-white uppercase tracking-wider">Kontak Kami</div>
            <ul className="space-y-3 text-xs">
              <li className="flex items-start gap-2.5 text-blue-100">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4 mt-0.5 shrink-0 text-blue-200">
                  <path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" />
                </svg>
                <span>Jl. Jend. Sudirman No.1, Kotabumi</span>
              </li>
              <li className="flex items-center gap-2.5 text-blue-100">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4 shrink-0 text-blue-200">
