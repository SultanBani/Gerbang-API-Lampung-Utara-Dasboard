...
export default function OpdCatalogPage() {
  ...
  const handleRequestAccess = async (endpointId) => {
    setSubmittingId(endpointId)
    try {
      const res = await api.post('/api/opd/access-requests', { endpoint_id: endpointId })
      showToast(res.data?.message || 'Permohonan hak akses API berhasil diajukan!')
      fetchData()
    } catch (err) {
      showToast(err?.response?.data?.message || 'Gagal mengajukan permohonan akses.')
    } finally {
  ...
  return (
    <div className="space-y-6">
              ...
              {filteredCatalog.map(ep => {
                ...
                return (
                  <div key={ep.id} className="group bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:border-blue-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-blue-900/10 flex flex-col justify-between">
                    ...
                    {/* Access Request Action Buttons & Status */}
                    <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
                      {isOwner ? (
                        <div className="w-full flex items-center justify-between">
                          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-[11px] font-extrabold">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-extrabold shadow cursor-pointer"
                          >
                            <ExternalLink className="w-3.5 h-3.5" />
                            <span>Tes API</span>
                          </a>
                        </div>
                      ) : ep.user_access_request ? (
                        ep.user_access_request.status === 'approved' ? (
                          <div className="w-full space-y-2">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-extrabold px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                            ...
                            {ep.user_access_request.api_key && (
                              <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 font-mono text-[10px]">
                                <span className="text-slate-500 truncate mr-2">Key: {ep.user_access_request.api_key}</span>
                                <button
                                  onClick={() => copyUrl(ep.user_access_request.api_key, `key-${ep.id}`)}
                                  className="text-blue-500 hover:underline font-bold shrink-0 cursor-pointer flex items-center gap-1"
                                >
                                  {copiedId === `key-${ep.id}` ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
                                  <span>{copiedId === `key-${ep.id}` ? 'Tersalin' : 'Salin Key'}</span>
                                </button>
                              </div>
                            )}
                          </div>
                        ) : ep.user_access_request.status === 'pending' ? (
                          <div className="w-full flex items-center justify-between p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-bold">
                            <span className="flex items-center gap-1.5">
                              <Clock className="w-4 h-4 animate-spin text-amber-500" />
                              <span>Permohonan Akses Pending...</span>
                            </span>
                            <span className="text-[10px] font-mono text-slate-400">Menunggu OPD</span>
                          </div>
                        ) : (
                          <div className="w-full flex items-center justify-between">
                            ...
                            <button
                              onClick={() => handleRequestAccess(ep.id)}
                              disabled={submittingId === ep.id}
                              className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-white font-bold text-xs rounded-xl shadow cursor-pointer"
                            >
                              Ajukan Ulang
                            </button>
                          </div>
                        ...
                        <div className="w-full flex items-center justify-between">
                          <span className="text-[10px] text-slate-400 font-semibold flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5 text-amber-500" /> Perlu Permohonan Hak Akses
                          </span>
                          <button
                            onClick={() => handleRequestAccess(ep.id)}
                            disabled={submittingId === ep.id}
                            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-60 text-white text-xs font-extrabold shadow-md shadow-blue-600/20 transition-all cursor-pointer"
                          >
                            {submittingId === ep.id ? (
      ...
      {toastMsg && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-slate-800 text-white font-extrabold text-xs px-5 py-3 rounded-2xl shadow-2xl z-[99999] flex items-center gap-2 border border-slate-700 animate-in fade-in slide-in-from-bottom-5">
