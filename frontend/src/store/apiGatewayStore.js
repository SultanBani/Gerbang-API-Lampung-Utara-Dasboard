...
export const store = reactive({
  ...
  // Access Controls: Map [appId_endpointId] -> boolean
  accessControls: {
  ...
  // Actions
  addApplication(app) {
    const newId = this.applications.length ? Math.max(...this.applications.map(a => a.id)) + 1 : 1
    const newApp = {
      id: newId,
      name: app.name,
    ...
    // Generate automatic API Key for new application
  ...
  addEndpoint(ep) {
    const newId = this.endpoints.length ? Math.max(...this.endpoints.map(e => e.id)) + 1 : 1
    const newEp = {
      id: newId,
  ...
  toggleAccess(appId, endpointId) {
    const key = `${appId}_${endpointId}`
    this.accessControls[key] = !this.accessControls[key]
  },

  generateNewKey(appId) {
    const apiKey = this.apiKeys.find(k => k.appId === appId)
    const randomKey = Array.from({length: 32}, () => Math.floor(Math.random()*16).toString(16)).join('')
    if (apiKey) {
