<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
            ...
            <p class="text-3xl font-extrabold text-slate-100 leading-none mb-1">{{ $appsCount ?? 8 }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
            ...
            <p class="text-3xl font-extrabold text-slate-100 leading-none mb-1">{{ $endpointsCount ?? 24 }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
            ...
            <p class="text-3xl font-extrabold text-slate-100 leading-none mb-1">{{ $activeKeysCount ?? 8 }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
            ...
            <p class="text-3xl font-extrabold text-slate-100 leading-none mb-1">{{ $requestsTodayCount ?? '4,821' }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
            ...
            <p class="text-3xl font-extrabold text-slate-100 leading-none mb-1">{{ $failedRequestsTodayCount ?? 37 }}</p>
                    ...
                    <tbody class="divide-y divide-slate-800/50">
                        @forelse($recentLogs ?? [] as $log)
                            <tr class="text-slate-300 hover:bg-slate-800/30 transition-colors">
                                <td class="py-3">{{ $log->created_at->format('H:i:s') }}</td>
                                <td class="py-3 font-semibold text-slate-200">{{ $log->application->name ?? 'Unknown' }}</td>
                                <td class="py-3 font-mono text-slate-400"><code>{{ $log->method }} {{ $log->url }}</code></td>
                                <td class="py-3 text-center">
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold {{ $log->status_code >= 400 ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400' }}">
                                        {{ $log->status_code }}
                                    </span>
                                </td>
                                <td class="py-3 text-right text-slate-400 font-mono">{{ $log->response_time_ms }}ms</td>
                            </tr>
                        @empty
